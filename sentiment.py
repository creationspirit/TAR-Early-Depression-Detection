...
class NLTKPreprocessor(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, stopwords=None, punct=None, lower=True, strip=True, excluded_stop_words=[]):
        """
...
@timeit
def build_and_evaluate(X, y, classifier=SGDClassifier, outpath=None, verbose=True, excluded_stop_words=[]):
    """
    ...
    def build(classifier, X, y=None):
        ...
        model = Pipeline([
            ('preprocessor', NLTKPreprocessor(excluded_stop_words=excluded_stop_words)),
...
if __name__ == "__main__":
    ...
    labels_file = open(DATASET_DIRECTORY + '/' + LABELS_FILENAME, 'r')
    label_dict = {}
    ...
    # initial dataset read
    X = []
    y = []
    pathlist = Path(DATASET_DIRECTORY).glob('**/*.xml')

    for path in pathlist:
        path_str = str(path)

        y.append(label_dict[re.sub(r".xml", "", os.path.basename(path_str))])

        tree = ET.parse(path_str)
        root = tree.getroot()

        individual = ''
        all_user_posts = root.findall('.//TITLE') + root.findall('.//TEXT')
